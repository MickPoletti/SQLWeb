CREATE DATABASE  IF NOT EXISTS `CarRental2019` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `CarRental2019`;
-- MySQL dump 10.13  Distrib 8.0.18, for macos10.14 (x86_64)
--
-- Host: localhost    Database: CarRental2019
-- ------------------------------------------------------
...
INSERT INTO `RATE` VALUES (1,0,480.00,80.00),(1,1,600.00,100.00),(2,0,530.00,90.00),(2,1,660.00,110.00),(3,0,600.00,100.00),(3,1,710.00,120.00),(4,0,685.00,115.00),(4,1,800.00,135.00),(5,0,780.00,130.00),(5,1,900.00,150.00),(6,0,685.00,115.00),(6,1,800.00,135.00),(1,0,480.00,80.00),(1,1,600.00,100.00),(2,0,530.00,90.00),(2,1,660.00,110.00),(3,0,600.00,100.00),(3,1,710.00,120.00),(4,0,685.00,115.00),(4,1,800.00,135.00),(5,0,780.00,130.00),(5,1,900.00,150.00),(6,0,685.00,115.00),(6,1,800.00,135.00);
...
/*!50001 VIEW `vrentalinfo` AS select `r`.`OrderDate` AS `OrderDate`,`r`.`StartDate` AS `StartDate`,`r`.`ReturnDate` AS `ReturnDate`,(to_days(`r`.`ReturnDate`) - to_days(`r`.`StartDate`)) AS `TotalDays`,`r`.`VehicleID` AS `VIN`,`v`.`Description` AS `Vehicle`,(case when (`v`.`Type` = 1) then 'Compact' when (`v`.`Type` = 2) then 'Medium' when (`v`.`Type` = 3) then 'Large' when (`v`.`Type` = 4) then 'SUV' when (`v`.`Type` = 5) then 'Truck' when (`v`.`Type` = 6) then 'VAN' end) AS `Type`,(case when (`v`.`Category` = 0) then 'Basic' else 'Luxury' end) AS `Category`,`r`.`CustID` AS `CustomerID`,`c`.`Name` AS `CustomerName`,`r`.`TotalAmount` AS `OrderAmount`,(case when (`r`.`Returned` = 0) then `r`.`TotalAmount` else 0 end) AS `RentalBalance` from ((`rental` `r` join `vehicle` `v`) join `customer` `c`) where ((`r`.`VehicleID` = `v`.`VehicleID`) and (`r`.`CustID` = `c`.`CustID`)) order by `r`.`StartDate` */;
...
-- Dump completed on 2019-11-23  0:27:10
